:- module(interpolantAutomaton_bck, [main/1], [dynamic]).
...
generateIFTA(IntF) :-
	...
	separate_constraints(Body, CsNl, B),
    separateLinearConstraints(CsNl, Cs, _),
	varset((H,Body), Vars),
	numbervars(Vars,0,_),
...
constraintTree(B,T,tree(B,Ys,Cs1,SubTrees,Zs, 0, K), K, K2) :-
	...
	varset((B,Bs1),Ws),
	setdiff(Ws,Ys,Zs),
	separate_constraints(Bs1,CsNL,Bs2),
    separateLinearConstraints(CsNL, Cs1, _),%non-linear is filtered out
	K1 is K+1,
