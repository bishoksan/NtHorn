:- module(insertNegProps, [main/1], [dynamic]).
%insert neg of the props given in the file
...
readPropFacts(_,end_of_file) :-
	!.
readPropFacts(S,(H:-C)) :-
    negate(C,CNeg), % CNeg=(C1;C2;C3;....)
    store_neg_props(H,CNeg),
	read(S,C1),
	readPropFacts(S,C1).
...
store_neg_props(H,CNeg):-
    varset(H,Xs),
    dummyCList(Xs,DCL),
    parse_n_store(CNeg,H,DCL).

parse_n_store((C1;C2),H,DCL):-
    !,
    parse_n_store(C1,H,DCL),
    parse_n_store(C2,H,DCL).
parse_n_store(C,H,DCL):-
    append(C,DCL,CsL),
    assertz(prop(H,CsL)).
